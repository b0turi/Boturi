...
VkShaderModule getShaderModule(const std::vector<char>& code) {
	...
	if (vkCreateShaderModule(Boturi::device, &createInfo, nullptr, &shaderModule) != VK_SUCCESS)
		Boturi::printError("failed to create shader module!");
...
VkResult makePipeline(const char * vertexShader, const char * fragmentShader, VkPipelineLayout layout, VkPipeline & pipeline)
{
	...
	viewport.x = 0.0f;
	viewport.y = 0.0f;
	viewport.width = (float)Boturi::extent.width;
	viewport.height = (float)Boturi::extent.height;
	viewport.minDepth = 0.0f;
	viewport.maxDepth = 1.0f;

	VkRect2D scissor = {};
	scissor.offset = { 0, 0 };
	scissor.extent = Boturi::extent;
	...
	multisampling.rasterizationSamples = Boturi::msaaSamples;
	...
	pipelineInfo.renderPass = Boturi::renderPass;
	pipelineInfo.subpass = 0;
	pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;

	VkResult result = vkCreateGraphicsPipelines(Boturi::device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline);

	vkDestroyShaderModule(Boturi::device, fragShaderModule, nullptr);
	vkDestroyShaderModule(Boturi::device, vertShaderModule, nullptr);
